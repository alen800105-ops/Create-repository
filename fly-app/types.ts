...
export interface SearchParams {
  ...
  minDays: number;    // Minimum trip duration
  maxDays: number;    // Maximum trip duration
  hasLuggage: boolean;
  outboundTime: TimeRange; // Custom Outbound departure range
  returnTime: TimeRange;   // Custom Return departure range
...
export interface TravelRecommendation {
  id: string;
  name: string;
  category: string; // Food, Spot, or Hotel
  description: string;
  location: string; // Address or searchable name
  subway: string;   // Subway access info
  priceLevel: string; // $, $$, $$$ or Actual Price range for hotels
  rating: string;     // e.g. 4.5
  tags: string[];
  bookingPlatform?: string; // e.g. Agoda, Airbnb (Only for hotels)
}
...
export interface TravelParams {
  ...
  foodTag?: string; // e.g. "Yakiniku"
  // Common
  centerLocation?: string; // If set, search around here
  keyword?: string;
}

export interface TravelResponse {
  recommendations: TravelRecommendation[];
  mapCenter: string; // Name of the place to center map on
}
